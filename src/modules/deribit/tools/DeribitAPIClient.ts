...
/** Single public trade row from Deribit `get_last_trades_by_currency_and_time` */
...
export class DeribitAPIClient {
  ...
  /**
   * Paginate through all option trades in [startMs, endMs] (Unix ms).
   * Deribit caps each request at 1000 rows; uses sliding end_timestamp cursor.
   */
  async fetchOptionTradesTimeRange(
    currency: 'BTC' | 'ETH',
    startMs: number,
    endMs: number,
    options?: { maxPages?: number; pageDelayMs?: number }
  ): Promise<{ trades: DeribitPublicTrade[]; truncated: boolean; pages: number }> {
    const maxPages = options?.maxPages ?? 80;
    const pageDelayMs = options?.pageDelayMs ?? 100;
    const seen = new Set<string>();
    const all: DeribitPublicTrade[] = [];
    let endCursor = endMs;
    let pages = 0;
    let lastHasMore = false;

    while (pages < maxPages) {
      const res = await axios.get(`${this.baseUrl}/get_last_trades_by_currency_and_time`, {
        params: {
          currency,
          kind: 'option',
      ...
      const batch: DeribitPublicTrade[] = res.data?.result?.trades ?? [];
      lastHasMore = res.data?.result?.has_more === true;
      ...
      const minTs = Math.min(...batch.map((t) => t.timestamp));
      if (minTs <= startMs) break;

      endCursor = minTs - 1;
      if (endCursor < startMs) break;

      await new Promise((r) => setTimeout(r, pageDelayMs));
    }

    const oldest = all.length ? Math.min(...all.map((t) => t.timestamp)) : endMs;
    const truncated = lastHasMore || (pages >= maxPages && oldest > startMs);

    return { trades: all, truncated, pages };
  }
}
