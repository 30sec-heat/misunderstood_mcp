...
import { OptionFlowAnalyzer } from './tools/OptionFlowAnalyzer.js';
...
export class DeribitModule extends BaseCryptoModule {
  ...
  protected setupTools() {
    ...
    this.addTool({
      ...
      description:
        'Desk-style read: option trade tape over a time window + OI snapshot + ELI12 narrative (puts vs calls, blocks, strike clusters). Uses public Deribit data only — cannot see OTC/counterparty.',
      inputSchema: {
        type: 'object',
        properties: {
          currency: {
          ...
          hours_back: {
            type: 'number',
            description: 'Lookback hours for trade tape (default 48, max 168)',
            default: 48
          },
          min_premium_usd: {
            type: 'number',
            description: 'Threshold (USD) for "notable prints" list — premium ≈ contracts × option price in coin × index',
            default: 75000
          },
          max_pages: {
            type: 'number',
            description: 'Max API pages (1000 trades each) when paginating tape; raise if truncated warning appears',
            default: 80
          }
        },
        required: []
      },
      handler: this.analyzeOptionFlows.bind(this)
    });
  }
  ...
  async analyzeVolatilitySurface(args: any) {
    try {
      ...
      if (analysisType === 'surface' || analysisType === 'all') {
        analysis.volatility_surface = this.buildVolatilitySurfaceFromAPI(options);
      }

      if (analysisType === 'term_structure' || analysisType === 'all') {
        analysis.term_structure = this.analyzeTermStructureFromAPI(options);
      }

      if (analysisType === 'skew' || analysisType === 'all') {
        analysis.volatility_skew = this.analyzeVolatilitySkewFromAPI(options, chainData.spot_price);
      }

      if (analysisType === 'all') {
        analysis.summary = this.generateVolatilitySummaryFromAPI(options, chainData.spot_price);
      }
  ...
  async analyzeOptionFlows(args: any) {
    try {
      const currency = (args.currency || 'BTC') as 'BTC' | 'ETH';
      const hoursBack = Math.min(Math.max(args.hours_back ?? 48, 1), 168);
      const minPremiumUsd = Math.max(args.min_premium_usd ?? 75000, 0);
      const maxPages = Math.min(Math.max(args.max_pages ?? 80, 1), 250);
      const endMs = Date.now();
      const startMs = endMs - hoursBack * 3600 * 1000;

      const [tape, chainData, historicalVol] = await Promise.all([
        this.apiClient.fetchOptionTradesTimeRange(currency, startMs, endMs, {
          maxPages,
          pageDelayMs: 100
        }),
        this.apiClient.getOptionsChain(currency),
        this.apiClient.getHistoricalVolatility(currency).catch(() => null)
      ]);

      const volAnalysis = this.optionsAnalyzer.analyzeOptionsChain(
        chainData.options,
        chainData.spot_price,
        historicalVol ?? undefined
      );

      const report = OptionFlowAnalyzer.buildReport({
        currency,
        spot: chainData.spot_price,
        hours: hoursBack,
        startMs,
        endMs,
        trades: tape.trades,
        ...
        pages: tape.pages,
        chainOptions: chainData.options,
        volAnalysis,
        minPremiumUsd
      });
      ...
      return {
        ...report,
        message: `Option flow + OI read: ${tape.trades.length} trades over ${hoursBack}h (${tape.pages} API pages)${
          tape.truncated ? ' — TAPE MAY BE INCOMPLETE' : ''
        }`
      };
    } catch (error) {
      console.error('Error in analyzeOptionFlows:', error);
      return {
        currency: args.currency || 'BTC',
        error: error instanceof Error ? error.message : 'Failed option flow analysis',
        message: 'Error analyzing option flows'
      };
    }
  }
